        ...
        >
          ...
          <div className="bg-slate-900/80 p-6 rounded-2xl border-2 border-slate-700 flex justify-center mb-6 relative z-10 shadow-inner">
            {avatarUrl && avatarUrl.startsWith('/') ? (
              <div className="w-48 h-48 mx-auto rounded-full overflow-hidden border-4 border-amber-300 shadow-[0_0_20px_rgba(251,191,36,0.6)] animate-[pulse_2s_ease-in-out_infinite] relative">
                <img 
                  src={avatarUrl} 
                  alt={name || avatarId} 
                  className={`w-full h-full object-cover ${avatarUrl.includes('cute_') ? 'scale-[1.3] origin-top' : ''}`}
                  style={{ objectPosition: avatarUrl.includes('cute_') ? 'center 15%' : 'center' }}
                />
              </div>
            ) : (
