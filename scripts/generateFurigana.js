...
const TERM_OVERRIDES = {
  ...
  "隕石": "いんせき",
  "一本": "いっぽん",
  // --- 読みが複数あって、子ども向けの読みに固定したい語 ---
  // 「日本」は にっぽん/にほん どちらも正しいが、学校で教わるのは にほん。
  // kuromojiは にっぽん を返すため、全102か所が にっぽん になっていた。
  "日本国憲法": "にほんこくけんぽう",
  "日本中": "にほんじゅう",
  "日本": "にほん",
  // ゴミの「分別」は ぶんべつ。kuromojiは ふんべつ（＝思慮の意味）を返していた。
  "分別": "ぶんべつ",
  // 「米」は文脈によって こめ/べい に分かれる。この教材では作物の話しか出てこないので
  // こめ に寄せる。国名の意味で使う語だけ先に押さえておく
  // （置換は長い語から順に効くので、下の "米" より先に適用される）。
  "米国": "べいこく",
  "欧米": "おうべい",
  "銘柄米": "めいがらまい",
  "米作り": "こめづくり",
  ...
  "お米": "おこめ",
  "米": "こめ",
  // かな＋漢字の交ぜ書きは辞書に載っておらず、1文字ずつ訓読みされてしまう
  // （こん虫→こんむし、よう虫→ようむし）。
  "こん虫": "こんちゅう",
  "よう虫": "ようちゅう",
  "せい虫": "せいちゅう",
...
async function main() {
  ...
  function generateSubjectKana(tsFilePath, exportName, outJsonPath) {
    ...
    for (const q of questions) {
      // 漢字を1文字も含まない問題はふりがなを作る必要がない。
      // 以前は「grade <= 2 ならスキップ」としていたが、小2には
      // 「観察」「郵便局」「消防署」「優先席」のような漢字が実際に含まれており、
      // ふりがなモードにしても漢字のまま表示されていた（実測で理科20問・社会20問）。
      // 学年ではなく「漢字が入っているか」で判定する。
      const hasKanji =
        KANJI_RE.test(q.word || "") ||
        KANJI_RE.test(q.reading || "") ||
        (q.choices || []).some(c => KANJI_RE.test(c));
