...
import { pullGachaItem, gachaRates, pullRichGachaItem, allRichGachaItems, richGachaRates, pullRichGacha2Item, richGacha2Rates, pullSpEquipmentGachaItem, spEquipmentGachaRates, pullRichEquipmentGachaItem, richEquipmentGachaRates, pullRichLadiesGachaItem, richLadiesGachaRates, all3000GachaRates, pullRichLadiesEquipmentGachaItem, richLadiesEquipmentGachaRates, all3000SpCombinedEquipmentRates, pullThemeGachaItem, themeGachaRates, pullLegendGachaItem, legendGachaRates } from "../../lib/gachaData";
...
const GachaMultiResultModal = dynamic(() => import("../../components/game/GachaMultiResultModal").then(mod => mod.GachaMultiResultModal), { ssr: false });
...
// テーマガチャ：中身は背景テーマ23種だけの高額ガチャ。
// 1回10,000PTと高いぶん、かぶったときは半額を返す（3000PTガチャの1000PT返還と同じ感覚にするため）。
const THEME_GACHA_COST = 10000;
const THEME_GACHA_REFUND = 5000;

// レジェンドガチャ：これまでの全ガチャの超激レア・神レアだけを集めた最上位ガチャ。
// 1回10万PTなので、かぶったときの返還も他の高額ガチャと同じ「半額」にそろえてある。
const LEGEND_GACHA_COST = 100000;
const LEGEND_GACHA_REFUND = 50000;

type GachaKind =
  | "regular" | "regular10" | "rich" | "rich2"
  | "rich_ladies" | "theme_gacha" | "legend"
  | "sp_equipment" | "rich_equipment" | "rich_ladies_equipment";

// ガチャごとの費用。確認ダイアログ・抽選・結果画面の「もう1回」ボタンが
// すべてここを見るようにしてある。
//
// 【重要】以前は「もう1回」ボタンだけ費用の分岐を別に書いていたため、
// テーマガチャ(1万PT)やレジェンドガチャ(10万PT)を引いたあとに
// 「3000 PT でもう1回」と誤表示され、3,000PTしか持っていない子には
// 押しても何も起きないボタンが出ていた。費用を変えるときはここだけ直すこと。
const GACHA_COSTS: Record<GachaKind, { cost: number; costType: "PT" | "SP" }> = {
  regular: { cost: 100, costType: "PT" },
  regular10: { cost: 1000, costType: "PT" },
  rich: { cost: 3000, costType: "PT" },
  rich2: { cost: 3000, costType: "PT" },
  rich_ladies: { cost: 3000, costType: "PT" },
  theme_gacha: { cost: THEME_GACHA_COST, costType: "PT" },
  legend: { cost: LEGEND_GACHA_COST, costType: "PT" },
  sp_equipment: { cost: 1000, costType: "SP" },
  rich_equipment: { cost: 3000, costType: "SP" },
  rich_ladies_equipment: { cost: 3000, costType: "SP" },
};
...
export default function ShopPage() {
  ...
  // Gacha state
  const [pullingType, setPullingType] = useState<GachaKind | null>(null);
  const [lastPullType, setLastPullType] = useState<GachaKind | null>(null);
  ...
  const [showGachaRates, setShowGachaRates] = useState<GachaKind | "all_3000" | "all_sp" | null>(null);
  ...
  const handleRequestGacha = (type: GachaKind) => {
    if (pullingType) return;
    // 表示名だけをここに置き、費用は GACHA_COSTS から引く（費用の二重管理を避けるため）
    const names: Record<GachaKind, { name: string; icon: string }> = {
      regular: { name: "ノーマルガチャ", icon: "⭐" },
      regular10: { name: "ノーマルガチャ10連", icon: "🎁" },
      rich: { name: "リッチガチャ 第1弾", icon: "🌟" },
      rich2: { name: "リッチガチャ 第2弾", icon: "🔥" },
      rich_ladies: { name: "ふわふわガチャ♡", icon: "🌸" },
      sp_equipment: { name: "SP装備ガチャ", icon: "⚔️" },
      rich_equipment: { name: "装備品リッチガチャ", icon: "🛡️" },
      rich_ladies_equipment: { name: "ふわふわ装備ガチャ♡", icon: "🎀" },
      theme_gacha: { name: "テーマガチャ", icon: "🖼️" },
      legend: { name: "レジェンドガチャ", icon: "👑" },
    };
    const target = { ...names[type], ...GACHA_COSTS[type] };
    if (!target) return;
  ...
  const pullGacha = async (type: GachaKind = "regular") => {
    ...
    const runEquipmentGacha = async (
      ...
      const ok = await updateUserDataAtomic(current => {
        ...
        // pull() は gachaData 内の共有アイテム配列の要素をそのまま返すため、必ずコピーしてから
        // gachaName / duplicated を書き込む。直接書き換えると、一度かぶったアイテムに
        // duplicated:true が残り、次に同じものを「新規で」引いても「再獲得」と誤表示される。
        resultItem = { ...pull(), gachaName };
        const userEquips = current.equipments || [];
    ...
    if (type === "rich_ladies") {
      ...
      const ok = await updateUserDataAtomic(current => {
        ...
        // 共有アイテムを直接書き換えないようコピーしてから使う（理由は runEquipmentGacha のコメント参照）
        resultItem = { ...pullRichLadiesGachaItem(), gachaName: "🌸 ふわふわガチャ♡" };
    ...
    if (type === "regular10") {
      ...
      setPullingType(type);
      setGachaResult(null);
      setGachaMultiResult(null);
      setShowGachaRates(null);

      let results: any[] = [];
      const ok = await updateUserDataAtomic(current => {
        if (current.pt < cost) { results = []; return null; }
        ...
        for (let i = 0; i < 10; i++) {
          ...
          const item: any = { ...pulled, gachaName: "🎁 通常ガチャ10連" };
          let duplicated = false;

          if (item.type === 'effect') {
            if (effects.includes(item.id)) duplicated = true;
            else effects.push(item.id);
          } else if (item.type === 'avatar') {
            if (avatars.includes(item.id)) duplicated = true;
            else avatars.push(item.id);
          } else if (item.type === 'title') {
            if (titles.includes(item.id)) duplicated = true;
            else titles.push(item.id);
          } else if (item.type === 'theme') {
            const themeEffectId = `theme_${item.id}`;
            if (effects.includes(themeEffectId)) duplicated = true;
            else effects.push(themeEffectId);
          }
        ...
        return { pt: current.pt - cost, xp, effects, avatars, titles };
      });
      ...
      setPendingGachaMultiResult(results);
      return;
    }

    const cost = GACHA_COSTS[type].cost;
    if (userData.pt < cost || pullingType) return;
    setPullingType(type);
    setGachaResult(null);
    setShowGachaRates(null);
    ...
    const ok = await updateUserDataAtomic(current => {
      ...
      if (type === "rich") {
        ...
        result = { ...pullLegendGachaItem(), gachaName: "👑 レジェンドガチャ" };
      } else {
      ...
      if (result.type === 'effect') {
        ...
        // 装備はこれまでSP専用ガチャだけの景品だったが、レジェンドガチャにも入るので
        // PT側のこの共通処理でも付与できるようにした。
        const owned = current.equipments || [];
        if (owned.includes(result.id)) duplicated = true;
        else updates.equipments = [...owned, result.id];
      }

      if (duplicated || result.type === 'xp') {
        if (type === "legend") {
          updates.pt = current.pt - cost + LEGEND_GACHA_REFUND;
          result.duplicated = true;
          result.refund = `${LEGEND_GACHA_REFUND.toLocaleString()} PT`;
        } else if (type === "theme_gacha") {
  ...
  return (
    <>
      ...
      {(pullingType === "rich" || pullingType === "rich2" || pullingType === "theme_gacha" || pullingType === "legend" || pullingType === "rich_equipment" || pullingType === "rich_ladies" || pullingType === "rich_ladies_equipment" || pullingType === "sp_equipment") && pendingGachaResult && (
        <RichGachaAnimation
      ...
      {pullingType === "regular10" && pendingGachaMultiResult && (
        <RegularGachaAnimation
          onComplete={() => {
            setGachaMultiResult(pendingGachaMultiResult);
            setPendingGachaMultiResult(null);
            setPullingType(null);
          }}
        />
      )}
      ...
      <GachaResultModal
        ...
        canPullAgain={
          lastPullType
            ? (GACHA_COSTS[lastPullType].costType === "SP" ? (userData.sp || 0) : userData.pt) >= GACHA_COSTS[lastPullType].cost
            : false
        }
        pullAgainLabel={
          lastPullType
            ? `${GACHA_COSTS[lastPullType].cost.toLocaleString()} ${GACHA_COSTS[lastPullType].costType} でもう1回`
            : undefined
        }
      />

      <GachaMultiResultModal
        isOpen={!!gachaMultiResult}
        results={gachaMultiResult}
        onClose={() => setGachaMultiResult(null)}
        onPullAgain={() => pullGacha("regular10")}
        canPullAgain={userData.pt >= 1000}
        pullAgainLabel="1000 PT でもう1回"
      />
    </>
  );
}
