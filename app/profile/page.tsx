...
type Tab = "avatars" | "equipments" | "titles" | "themes" | "effects";
                    ...
                    {allEquipments.map(eq => {
                      ...
                      return (
                        <div key={eq.id} className={`game-panel-light p-4 flex flex-col justify-between gap-3 ${isEquipped ? 'border-emerald-500 bg-emerald-50/90' : !isOwned ? 'opacity-85' : ''}`}>
                          <div className="flex items-center gap-4">
                            <div 
                              className="text-5xl w-16 h-16 flex items-center justify-center bg-slate-900/80 rounded-2xl border-2 border-amber-300 shadow-md flex-shrink-0 cursor-pointer hover:scale-110 transition-transform"
                              onClick={() => setPreviewingEquipmentModal(eq.id)}
                            >
                              {eq.icon.startsWith('/') ? <img src={eq.icon} alt="eq" className="w-full h-full object-cover rounded-xl" /> : eq.icon}
                            </div>
